...
| 🎬 **电影级运镜与布光模板** | [cinematic-shots.md](references/cinematic-shots.md) | 50 套导演级运镜 + 10 种人像布光方案 |
| 🎨 **深度美学约束** | [aesthetic-constraints.md](references/aesthetic-constraints.md) | Octane 级渲染品质 + 冷暖色调 + 场景色彩词库 + 艺术风格锁定 |
...
> 另有辅助参考文档：[image-to-prompt.md](references/image-to-prompt.md)（路径 C 核心方法）、[creative-strategy.md](references/creative-strategy.md)（创意策略）、[long-video-strategy.md](references/long-video-strategy.md)（长视频策略 + 分镜六步法 + 时间分配）、[expression-prompts.md](references/expression-prompts.md)（表情词库）、[vocabulary.md](references/vocabulary.md)（运镜修饰词 + 镜头标签系统）、[examples.md](references/examples.md)（更多示例）、[cli-integration.md](references/cli-integration.md)（CLI 执行）、[platform-specs.md](references/platform-specs.md)（平台参数）、[storyboard-driven.md](references/storyboard-driven.md)（分镜板驱动路径 D）。
...
1. **≤15秒（路径 A）**：不要讲故事，做一个"不可能的瞬间"。前2秒必须抓人。概念越简单越好，一个视频只做一件事。**短剧/剧情向 15 秒例外**：如果有情绪叙事需求（人物表情+对白），仍走路径 A，但需额外调用 [expression-prompts.md](references/expression-prompts.md) 翻译情绪为物理形变、[aesthetic-constraints.md](references/aesthetic-constraints.md) Part 6/7 锁定色彩与风格。
2. **>15秒（路径 B）**：不要指望一条提示词一步到位。必须走完整前期流程：角色卡片图 → 分镜脚本 → 分镜参考图 → 逐镜头生成 → 后期拼接。
3. **图片驱动（路径 C，v3.1·极简优先 + 边界条件）**：用户丢图进来想做视频。**核心原则：信任 Seedance 2.0 对图的理解能力，文字只补 4 件事**——①图本身没有的（时长/比例/运镜大方向）；②图存在但需强化的（节奏/情绪 hook/关键瞬间）；③图里需要规避的（不复刻网格/原图水印/分镜板格子）；④保险的硬约束（无文字水印/不可识别真人面孔/合规风险）。**Prompt 控制在 300-500 字符**（路径 C 极简策略适用范围；常规提示词上限 2000 字符，目标 ≤1900 留 100 余量）。**⚠️ 极简策略有边界**（2026-05-23 实测：通过率 25%、还行 50%、翻车 25%）：✅ 多宫格分镜板/凝视模式简单情绪/抖音爆款（需显式 hook）适用；❌ 反应模式（惊讶/慌乱/紧张感）需子节拍展开不可极简；❌ 多宫格画风差异大的叙事拼接 Seedance 无法弥合，需改"心境拼贴"或换图。**Seedance prompt 上限 2000 字符**（Python `len()` **实测**，不准凭中文字感觉估算 —— 详见 [image-to-prompt.md](references/image-to-prompt.md) "极简优先铁律"+"极简优先策略·边界条件验证"+"字符数实测铁律"小节）。
4. **分镜板驱动（路径 D，v0.1 试运行）**：用户上传 N 宫格分镜板/漫画分镜，要求"按这几格生成"。需先选 D-1（≤4 格→单条多段时间戳）或 D-2（≥5 格→多条独立 prompt+剪辑节奏建议），完整方法论见 [storyboard-driven.md](references/storyboard-driven.md)。
...
| 11 | **人物表情控制** | `[角色] + {物理形变提示词} + 传达{情绪目的}` | 把抽象情绪翻译成可见的面部肌肉/眼神/嘴唇变化（见 [expression-prompts.md](references/expression-prompts.md)） |
...
> 不要只写"悲伤、愤怒、害怕"等抽象情绪词——把情绪翻译成**可见的面部肌肉、眼神、嘴唇、呼吸、姿态变化**，AI 才能精准演绎。

完整词库（10 大类 60+ 种表情的物理形变提示词 + 组合模板 + 与四维编码联动表）见 [expression-prompts.md](references/expression-prompts.md)。
...
**台词时间估算公式**（按语速精准估时，详见 [long-video-strategy.md](references/long-video-strategy.md) "分镜时间分配策略"）：

| 语速类型 | 公式 | 适用 |
|:---|:---|:---|
| 正常语速 | `round(字数÷8)+1秒` | 普通对白、解释、问答 |
| 争吵/急促 | `round(字数÷10)+1秒` | 语速快、打断、情绪激烈 |
| 深情/遗言 | `round(字数÷5)+1秒` | 慢速表达、停顿多、情绪重 |
| VO 旁白 | `round(字数÷7)+1秒` | 旁白、内心独白 |

> 优先冲突原则：镜头同时含台词和动作时，取两者计算结果中的**较大值**。单镜严格限制在 12 秒以内。
...
详细分段策略、分镜时间分配策略（台词估时公式/按情绪类型分配/各时长分镜模板）、分镜创作六步法和输出模板见 [long-video-strategy.md](references/long-video-strategy.md)。
...
完整色调 HEX 值、渲染品质三档约束词、项目类型完整速查表、四套美学提示词模板、**短剧/剧情向场景色彩词库（100+ 种色彩方案 + 权重公式）**以及**11 种艺术风格完整提示词前缀 + 风格禁混表**，见 [aesthetic-constraints.md](references/aesthetic-constraints.md)。
...
### 艺术风格锁定·媒介互斥规则（短剧/剧情向补充）

> 短剧和剧情类视频尤其需要**先锁定一种主风格**，再写角色、场景、镜头、表情和色彩。风格锚词应放在提示词前段。

**五种媒介互斥**（不可无说明混用）：

| 媒介 | 包含风格 | 核心视觉 |
|:---|:---|:---|
| **真人** | 真人古风写实、真人都市、真人都市写实轻量变体 | 真人摄影、影视级纪实 |
| **二次元** | 90年代日式动画、国风二次元新国潮、成熟都市言情二次元 | 手绘平涂、赛璐璐 |
| **3D** | 3D动画渲染、国风3D渲染、国风赛博3D | PBR材质、体积光、高精度建模 |
| **定格** | 定格动画黏土 | 黏土肌理、手指压痕、手工定格 |
| **扁平** | 2D扁平风 | 几何造型、纯色色块、无阴影 |

**关键规则**：
1. 单个项目优先锁定**一种主媒介**；若剧情需要风格转换，应在分镜中明确"现实段/回忆段/梦境段/手机画面"等边界
2. 不混用互斥媒介词：真人写实、二次元、3D渲染、黏土定格、扁平设计不能无说明地堆在同一镜头里
3. 风格词只负责"画面媒介和审美基因"，具体情绪仍调用人物表情、镜头语言和场景色彩词库

> 完整的 11 种风格提示词前缀 + 风格禁混明细表 + 选型建议，见 [aesthetic-constraints.md](references/aesthetic-constraints.md) Part 7。
...
**运镜修饰词**（Smooth / Slow / Cinematic / Handheld / Aerial / POV 等）见 [vocabulary.md](references/vocabulary.md)（含镜头标签系统 + 功能分类镜头词库 + 高级组合速查）。

**电影级运镜模板**（50 套导演级镜头 + 10 种人像布光方案，每条含机位/焦段/光影/叙事逻辑）见 [cinematic-shots.md](references/cinematic-shots.md)。
...
- 完整方法论见 [storyboard-driven.md](references/storyboard-driven.md)（试运行中，建议先用 1-2 个 case 实测验证再深用）
...
6. **艺术风格/媒介**（短剧/剧情向必确认）：真人 / 二次元 / 3D / 黏土 / 扁平（详见 [aesthetic-constraints.md](references/aesthetic-constraints.md) Part 7）

### Step 3：生成提示词

- **≤15秒（路径A）**：生成 **2-3 个不同风格版本** 供选择，每个版本附首帧图生成建议。涉及人物情绪时，调用 [expression-prompts.md](references/expression-prompts.md) 把抽象情绪翻译为物理形变
- **>15秒（路径B）**：按生产流水线依次输出各阶段产物。先用分镜创作六步法规划镜头叙事（见 [long-video-strategy.md](references/long-video-strategy.md)），再写分段提示词
- **图片驱动（路径C）**：先读图分析 + Hook 候选，用户选定后再出完整提示词（必要时同时给扩图建议）
- **商业级/电影级品质**：从 [cinematic-shots.md](references/cinematic-shots.md) 选用编号运镜模板和布光方案
- 每个提示词**可直接复制到即梦平台使用**
...
- [ ] **短剧/剧情向**：如使用场景色彩词库，是否选择了与情绪用途匹配的色彩方案（见 [aesthetic-constraints.md](references/aesthetic-constraints.md) Part 6）
- [ ] **艺术风格锁定**：主风格已锁定，未混用互斥媒介词（真人/二次元/3D/黏土/扁平不可无说明混用，见 Part 7 风格禁混表）

**人物表情控制（短剧/剧情向）：**
- [ ] 抽象情绪词已翻译成可见的面部肌肉/眼神/嘴唇/身体反应（见 [expression-prompts.md](references/expression-prompts.md)）
- [ ] 表情与景别/机位/运镜协同（如愤怒配低角度仰拍、羞涩配平视缓慢推近）
- [ ] 重要表情至少停留 1.5 秒，给观众看清楚
...
- [ ] `@图片N / @视频N` 后紧跟了角色名或解释词
- [ ] **电影级模板**：商业级/电影级场景是否从 [cinematic-shots.md](references/cinematic-shots.md) 选用了编号运镜/布光模板

**分镜设计（长视频必检）：**
- [ ] 是否按分镜创作六步法规划了镜头叙事（跟随对象→运动路线→镜头解释空间→主体关系→停顿点→结尾信息变化）
- [ ] 每个镜头是否只有一个核心动作或信息变化
- [ ] 关键动作至少 2 秒，重要表情至少停留 1.5 秒
- [ ] 台词镜头已按语速公式估时（`round(字数÷语速)+1`），同镜含台词+动作时取较大值
- [ ] 单镜严格控制在 12 秒以内
- [ ] 结尾有明确的信息变化（揭示/反转/危险/悬念），不是只停在氛围画面
- [ ] 每 10-15 秒出现一次冲突升级、线索揭示或情绪变化
